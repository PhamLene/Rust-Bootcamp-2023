...
fn is_palindrome(word: &str) -> bool {
    let len = word.len();
    let vec_char: Vec<_> = word.to_lowercase().chars().collect();
    for i in 0..len / 2 {
        if vec_char[i] != vec_char[len - 1 - i] {
            return false;
        }
    }
    true
...
fn count_char_occurrences(string: &str, ch: char) -> usize {
    let len = string.len();
    //count_char_occurrences(string, ch);
    let vec_char: Vec<_> = string.to_lowercase().chars().collect();
    let mut count = 0;
    for i in 0..len {
        if vec_char[i] == ch {
            count = count + 1;
